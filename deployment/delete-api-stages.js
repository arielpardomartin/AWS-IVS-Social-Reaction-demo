...
const deleteStages = async () => {
  try {
    ...
    const socialReactionsApiId = findOutput(cloudformationOutputs, 'SocialReactionsApiId');

    // Delete stages
    AWS.config.update({ region: awsRegion });
    const apigatewayv2 = new AWS.ApiGatewayV2();

    const deleteStageSocialReactionsParams = {
      ApiId: socialReactionsApiId,
      StageName: 'demo',
    };
    await apigatewayv2.deleteStage(deleteStageSocialReactionsParams).promise();
    console.log('\nSocial Reactions REST API "demo" stage deleted!');
